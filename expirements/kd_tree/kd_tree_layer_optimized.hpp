#ifndef KD_TREE_LAYER_OPTIMIZED_HPP_
#define KD_TREE_LAYER_OPTIMIZED_HPP_
...
namespace spatial_lib_layer_optimized {
...
class KD_Tree_Base {
	...
	void link_tree() {
		...
		std::vector<Bounds> current_bounds;
		...
		while ( !current_bounds.empty() ) {
			...
			// 	max_stack_size = current_bounds.size();
			// }
		}
		// std::cout << "layer max stack size: " << max_stack_size << std::endl;
	}
