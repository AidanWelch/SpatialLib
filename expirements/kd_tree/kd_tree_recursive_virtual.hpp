#ifndef KD_TREE_RECURSIVE_VIRTUAL_HPP_
#define KD_TREE_RECURSIVE_VIRTUAL_HPP_
...
namespace spatial_lib_recursive_virtual {
...
template <KDTreeVectorDataConstraint T> class KD_Tree<T> : public KD_Tree_Base<T> {
	...
	explicit KD_Tree( std::vector<T>& data_vector )
		: KD_Tree_Base<T>(), dimensions( data_vector[0].coordinates.size() ) {
...
template <KDTreeArrayDataConstraint T> class KD_Tree<T> : public KD_Tree_Base<T> {
	...
	public:
	explicit KD_Tree( std::vector<T>& data_vector ) : KD_Tree_Base<T>() {
		KD_Tree_Base<T>::balance_tree( &data_vector );
	};
